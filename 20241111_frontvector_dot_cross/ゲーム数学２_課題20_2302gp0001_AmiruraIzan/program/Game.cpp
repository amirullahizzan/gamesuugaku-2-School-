...
{
	ground.Update();
	//	プレイヤーの更新処理（マウス左ドラッグで回転・ＷＡＤキーで移動）
	player.Update();
	for( int i = 0; i < NPC_MAX; i++ )
	{
		npc[i].Update(player.m_pos, camera.m_rot);
	}
	//	カメラの更新処理（プレイヤーの後ろに座標を設定する）
...
void GameRender()
{
	...
	DrawString( 16, 32, "マウス右ドラッグ：カメラの向きに対してカメラ座標の左右移動（離したら戻る）", GetColor( 255, 255, 0 ) );
	DrawString( 16, 48, "Ｗキー　：プレイヤー前方向に移動", GetColor( 255, 255, 255 ) );
	DrawString( 16, 64, "ＡＤキー：プレイヤー移動（向きを変えずに左右移動）", GetColor( 255, 255, 255 ) );
	DrawString( 16, 80, "ＱＥキー：向きを変えずに左右移動（ダッシュ）", GetColor( 255, 255, 0 ) );
}
//---------------------------------------------------------------------------------
//	終了処理
//---------------------------------------------------------------------------------
